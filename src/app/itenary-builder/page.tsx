    ...
    <div className="bg-white h-[100vh] w-[100vw] flex justify-center  item-center">
      <div className="w-[80%] h-full p-10">
        <div className="flex space-x-30 ">
          <div className="flex rounded-3xl w-full overflow-x-scroll bg-gray-200 justify-between items-center">
            {noOfDays?.map((no: any) => {
              let value = `Day ${no}`
              if (no === 0) {
                value = `Cover`
              }
              return <div onClick={() => handleSelect(no)} className={`bg-gray-200 p-3 cursor-pointer w-full rounded-3xl flex justify-center items-center ${selectedDay === no ? 'bg-blue-400' : ''}`}>
